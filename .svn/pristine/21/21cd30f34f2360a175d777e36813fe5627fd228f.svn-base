  ...
  <div class="detali">
    ...
    <div class="detalisBox">
      ...
      <div class="detaliTitle" v-show="dairyType == 1">
        ...
        <div class="comtleB">
          <span class="comtleL" v-text="reference"></span>
      ...
      <div class="sz" v-show="sz" @click="goUrl">
        <div class="daoBox">
          <div class="daoLeft">
            <div class="banner-item daoLeft" :style="'background-image: url('+dao.picPath+')'"><img
              src="../../assets/daoImg.png"></div>
          </div>
          <div class="daoRight">
    ...
    <div class="newInput" v-show="newInput" @click="noStop">
      <textarea type="text" id="newInput" class="cdf" placeholder="把您的建议写在这里"></textarea>
      <div class="fa" @click="subOpin">发表</div>
    </div>
    <!--GB话题底部评论栏-->
    <form class="gbhui" v-show="sub == 1">
      <input type="text" id="Gbcon" class="cdf" placeholder="写回答" v-model="message">
    </form>
    <!--底部评论栏-->
    <div class="foodCom" v-show="dairyType == 1">
      ...
      <div class="foodL">
        ...
        <form>
          <input type="text" id="comCon" class="cdf" placeholder="写评论" @click="writeInput">
        </form>
      </div>
      <!--右侧已阅读数-->
      <div class="foodR">
...
<style>
  ...
  * {
  ...
  .kkk .autoImg {
    background: url("../../assets/mbydz2x.png") no-repeat 0 0/100% 100%;
    margin-top: -4px;
    margin-left: 5px;
    vertical-align: top;
    display: inline-block;
...
//  接收新闻id／作者id／文章标题／文章来源／发布时间／文章作者名字/文章描述/文章类型/是否收藏过的状态/文章封面图
      ...
      _this.reference = this.$route.params.reference;
...
//          判断用户是否被禁言
          $.ajax({
            type: 'GET',
            url: url + '/artcle/limitReply',
            data: {uid: userId},
            success: function (response) {
              if (response.error == 0) {
                console.log(response.isGag);
//                0可以发言，1被禁言
                if (response.isGag == 0) {
//                判断评论内容是否为空
                  if (ping != "") {
...
//      点击导购
      goUrl () {
        window.location.href = this.dao.url;
      }
    },
    computed: {
      text() {
