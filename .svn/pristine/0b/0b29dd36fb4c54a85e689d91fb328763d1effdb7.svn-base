<template>
  <div class="page">
    <!--首页-->
    <keep-alive>
      <component :is="currentView" :fromtype="froms">
        <!-- 非活动组件将被缓存！ -->
      </component>
    </keep-alive>
    <!--资讯／发现／我-->
...
<script>
  ...
  export default {
    ...
    beforeRouteEnter (to, from, next) {
      var fromObj = from.path;
      if (fromObj.split("/")[9]) {
        window.localStorage.index = fromObj.split("/")[9];
      }
      next(vm => {
        if (from.path == "/meDe" || from.path == "/collect" || from.path == "/count" || from.path == "/feed" || from.path == "/about" || from.path == "/login") {
          vm.currentView = me;
    ...
    beforeDestroy () {
      window.localStorage.removeItem("shab");
    },
    components: {
      New,
      find,
